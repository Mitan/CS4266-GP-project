...
class Graph(object):
    def __init__(self, graph__mapping={}):
        self.__graph_mapping = graph__mapping
    ...
    def findAllPaths(self, start, end, path=[]):
        ...
        for vertex in graph[start]:
            if vertex not in path:
                extendPaths = self.findAllPaths(vertex, end, path)
...
Y = []
Z= []
with open('Raw-count-data-major-roads-derby.csv', 'r') as f:
    reader = csv.reader(f)
    for row in reader:
        if i>100000:
            break
...
"""print(sys.getsizeof(graph.vertices()))
print(list(range(1,sys.getsizeof(graph.vertices()))))
...
print(graph.findAllPaths('-1.4595627578652.9487739173', '-1.4340804230152.951262286'))
